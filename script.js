...
/*function for adding elements to sidebar collections*/
function dispCollection(){
...
for(let a=0;a<collections.length;a++)
{
   ...
   {
    if(collections[a].collection==obj[j])
    {
          var d=`<li>${collections[a].movie}</li>`;
          document.getElementById(obj[j]).innerHTML+= d; 
    }
   }
}
}
